...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5000,
        help="Matrix size: N x N for sparse, N x M for dense (default: 5000)",
    )
    parser.add_argument(
        "--M",
        type=int,
    ...
    # --sparse 플래그 추가: sparse-dense 연산 평가 수행
    parser.add_argument(
        "--sparse",
        action="store_true",
...
def compare_accuracy_and_speed(
    N, M, num_iter, device="cuda", allow_tf32_flag=False, scale=1.0, sparse_mode=False
):
    """
    ...
    if not sparse_mode:
        # ----- Dense 모드 -----
        print("Running dense-dense multiplication test.")
        # 1) FP64 기준 dense 행렬 생성
        A_np_64 = np.random.randn(N, M).astype(np.float64)
        A_t64 = torch.from_numpy(A_np_64).to(device=device, dtype=torch.float64)

        # 2) FP64 기준 결과 (A_t64.T @ A_t64) 계산
        with torch.no_grad():
            ref_start = time.time()
            ref_result = A_t64.T @ A_t64  # FP64 dense matmul
            if device == "cuda":
                torch.cuda.synchronize()
            ref_time = time.time() - ref_start

        print("Reference (FP64) dense matmul done. (Not part of main benchmark timing)")

        # TF32 기본값 저장 (이후 토글을 위해)
        ...
        def test_dtype(dtype_name):
            """
            주어진 dtype에 대해:
              - A_t64를 해당 dtype으로 변환,
              - 필요시 scale 적용,
              - allow_tf32 토글,
              - matmul 반복 수행 시간 측정,
              - FP64 결과(ref_result)와의 상대 오차 계산.
            """
            ...
            def matmul_op():
                if scale != 1.0:
                    return (A_local.T @ A_local) / (scale**2)
                else:
                    return A_local.T @ A_local
        ...
        # 테스트할 dtype 목록 (dense 모드)
        results = {}
        dtypes_to_test = ["fp64", "fp32"]
        if device == "cuda":
            dtypes_to_test.append("fp16")
            if allow_tf32_flag:
                dtypes_to_test.append("tf32")

        for dt in dtypes_to_test:
            t, err = test_dtype(dt)
            results[dt] = (t, err)

        print("\n==== Final Summary (Dense-Dense) ====")
        for dt in dtypes_to_test:
            t, err = results[dt]
        ...
        print(f"Dense matrix shape: {N} x {M}")
        print(f"Sparse matrix shape: {N} x {N}")

        # 1) dense 행렬 (FP64) 생성 (A_dense: shape N x M)
        A_dense_np_64 = np.random.randn(N, M).astype(np.float64)
        A_t64_dense = torch.from_numpy(A_dense_np_64).to(device=device, dtype=torch.float64)

        # 2) sparse 행렬 (FP64) 생성 (A_sparse: shape N x N)
        A_sparse_np_64 = init_sparse_matrix(N, np.float64)
        A_sparse_torch_64 = scipy_to_torch_sparse(A_sparse_np_64).to(device=device, dtype=torch.float64)
        ...
        print("Reference (FP64) sparse-dense matmul done. (Not part of main benchmark timing)")
...
def main_accuracy_test():
    ...
    if args.sparse:
        print(f"Using sparse-dense multiplication:")
        print(f"  Dense matrix shape: {args.N} x {args.M}")
        print(f"  Sparse matrix shape: {args.N} x {args.N}")
    else:
        print(f"Using dense-dense multiplication: Matrix shape: {args.N} x {args.M}")
