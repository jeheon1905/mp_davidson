    ...
    for block in timer_blocks:
        ...
        for op_name, op_dtype, time, count in operations:
            record = {
                ...
                'use_dense_proj': dense_proj_match.group(1).lower() == 'true' if dense_proj_match else None,
                'use_dense_kinetic': dense_kinetic_match.group(1).lower() == 'true' if dense_kinetic_match else None
            }
...
def create_performance_table(df: pd.DataFrame, use_dense_proj=False):
    ...
    # DataFrame 생성
    formatted_table = pd.DataFrame(result_rows, index=pd.MultiIndex.from_tuples(result_index))
    
    # 열 이름을 정수로 정렬
    if not formatted_table.empty:
        formatted_table = formatted_table.reindex(sorted(formatted_table.columns), axis=1)
...
if __name__ == "__main__":
    ...
    try:
        table_false, dp_pivot_false, other_pivot_false = create_performance_table(df, use_dense_proj=False)
        if not table_false.empty:
            print_performance_table(table_false, "Performance Comparison (use_dense_proj=False)")
            table_false.to_csv('performance_dense_proj_false.csv')
    ...
    try:
        table_true, dp_pivot_true, other_pivot_true = create_performance_table(df, use_dense_proj=True)
        if not table_true.empty:
            print_performance_table(table_true, "Performance Comparison (use_dense_proj=True)")
            table_true.to_csv('performance_dense_proj_true.csv')
